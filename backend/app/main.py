...
from .models import appeal, email_verification, job, listing_unlock, payment, provider_listing, review, user
from . import email_auth, fulfillment, payments as payment_service
        ...
        if "email_verified" not in columns:
            ...
            conn.execute(
                text(
                    ...
                    "WHERE email IS NOT NULL AND email NOT LIKE '%@blackrabbit.local'"
                )
            )
...
@app.post("/auth/email/request-code", response_model=auth_schema.EmailCodeResponse)
def request_email_verification_code(
    payload: auth_schema.EmailCodeRequest, db: Session = Depends(get_db)
):
    code, demo_mode = email_auth.request_verification_code(
        db,
        email=payload.email,
        name=payload.name,
        phone=payload.phone,
    ...
    return auth_schema.EmailCodeResponse(
        message="Verification code sent. Check your inbox.",
        email=str(payload.email).lower(),
        expires_in_minutes=email_auth.CODE_TTL_MINUTES,
        demo_mode=demo_mode,
        dev_code=code if demo_mode else None,
    )


@app.post("/auth/email/verify", response_model=auth_schema.EmailVerifyResponse)
def verify_email_code(payload: auth_schema.EmailVerifyRequest, db: Session = Depends(get_db)):
    db_user, is_new_user = email_auth.verify_code_and_upsert_user(
        db,
        email=payload.email,
        code=payload.code,
    )
    return auth_schema.EmailVerifyResponse(
        user=_serialize_user(db, db_user),
...
def update_user(
    ...
    if updates.get("is_provider") and not db_user.email_verified:
        raise HTTPException(
            status_code=403,
            detail="Verify your email before enabling provider mode.",
        )
